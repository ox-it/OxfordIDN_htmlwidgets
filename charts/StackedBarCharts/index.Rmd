...
<a href="https://github.com/ox-it/LiveData_htmlwidgets/tree/gh-pages/charts/StackedBarCharts" target="blank">  
...
```{r, echo=FALSE}
library(highcharter)
stacked_bar_chart_df <- data.frame(
  "Countries" = c(
...
stacked_hc_chart(data = stacked_bar_chart_df,
...
Where the there are multiple measures (numerical values) for each category (or *dimension*), there are therefore two interesting stacked bar charts that can be generated:

- Total number of observations per category, split by measure
- Percentage of observations per measure for each category.

Note that this template covers both how to build such a stacked bar chart inside of an HTML RMarkdown file and how to functionalise the code so as to conveniently switch between different categories and metrics in a Shiny app.

# Import and Clean Data

This tutorial uses the `data.frame` below, note that the number have been specially chosen to have both varying range and variance for later comparison purposes.
...
library(highcharter)
highchart() %>%
  hc_xAxis(categories = stacked_bar_chart_df[,categories_column],
           title = categories_column) %>%
  hc_add_series(name = measure_columns[1],
                data = stacked_bar_chart_df[,measure_columns[1]]) %>%
  hc_add_series(name = measure_columns[2],
                data = stacked_bar_chart_df[,measure_columns[2]]) %>%
  hc_add_series(name = measure_columns[3],
                data = stacked_bar_chart_df[,measure_columns[3]]) %>%
  hc_add_series(name = measure_columns[4],
                data = stacked_bar_chart_df[,measure_columns[4]]) %>%
  hc_add_series(name = measure_columns[5],
                data = stacked_bar_chart_df[,measure_columns[5]]) %>%
...
```{r}
lapply(measure_columns, function(x){
  nchar(x)
})
```
...
```{r}
var_vector <- unlist(lapply(measure_columns, function(x)var(stacked_bar_chart_df[,x])))
...
1) Ordering operations should be applied within the visualisation function, and not applied to the data as the data's original ordering may not only be important but should be considered an instrinsic property of the data
...
Ordering of series in the `highcharter` library is set using the `index` value of each series - as a small example. Note that the index must start from 0.
...
invisible(lapply(1:length(measure_columns), function(colNumber) {
  generated_chart <<-
    hc_add_series(
      hc = generated_chart,
      name = measure_columns[colNumber],
      data = stacked_bar_chart_df[, measure_columns[colNumber]],
...
The steps above can be combined into a easy to use utility function:
