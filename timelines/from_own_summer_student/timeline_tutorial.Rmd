...
The Chunk here shows how to import data, here from figshare, and then the second part shows how to convert the date columns in the data into date format in R so they can be used as continious data  on a timeline.
```{r, echo=TRUE}
...
This chunk shows how to change the Prime Ministers names to order them chronologically rather than alphabetically - which is how they'll automatically order.
```{r, echo=TRUE}
## Find earliest date for each policy
earliest_date_by_Prime_Minister <-
  timeline_data2[timeline_data2$Start.Date == ave(timeline_data2$Start.Date, timeline_data2$Prime.Minister, FUN =
                                                  min),]
...
timeline_data2$Prime.Minister <-
  factor(timeline_data2$Prime.Minister, levels = rev(as.character(unique(earliest_date_by_Prime_Minister$Prime.Minister)))) 
...
This chunk shows how to get rid of any rows with an n/a value
```{r, echo=TRUE}

prime_minister_data<-timeline_data2[!is.na(timeline_data2$End.Date) & !is.na(timeline_data2$Start.Date),]
```

This chunk shows how to use ggplot to create a static timeline with the dates on the x axis, the names on the y axis, the colour is a way in which the data will be classified by colour so in this case prime ministers of different political parties will have different coloured bars, the geom-segment determines the width of the bar and the xlab and ylab are labels for the x and y axis.
```{r, echo=TRUE}
library(ggplot2)
ggplot(prime_minister_data, aes(x=Start.Date, xend=End.Date, y=Prime.Minister, yend=Prime.Minister, colour=Political.Party)) + geom_segment(size=3) + 
xlab("Date") + ylab("Prime Minister")
```

This chunk shows how to convert the static timeline into an interactive timeline using plotly.
```{r, echo=TRUE}
library(plotly)
ggplotly(ggplot(prime_minister_data, aes(x=Start.Date, xend=End.Date, y=Prime.Minister, yend=Prime.Minister, colour=Political.Party)) + geom_segment(size=3) + 
xlab("Date") + ylab("Prime Minister"))
```
